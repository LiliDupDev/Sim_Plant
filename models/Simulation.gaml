...
{image_file floor<-file("../images/tierra.jpg");
	...
	//  Belong to the test start 2
	/*reflex target when: target = nil {
		float min_distance <- #infinity;
		float min_temp <- 0.0;		
		ask agents of_species planta {
			loop myagent over: agents {	
				//write min_temp;
				
				min_temp <- myself distance_to myagent ;			
				if (min_temp < min_distance){
					myself.target <- myagent;
					min_distance <- min_temp; 
				} 
				write min_distance;
				if (myself.target != nil) {
					//write (myself.target as planta).id;
				} 				
			}			
		}
	}
	// Belong to the test end 2*/
...
/*list<planta> plants <- planta where (each distance_to self <= 5.0);
	...
	reflex update
	{
		...
		agent agentB<-planta closest_to({2,3});
		do goto target:agentB.location speed:vel_proteina;
	}
...
experiment name type: gui {
	...
	parameter "Temperatura" 		var: temp_env ;			//LD
	parameter "Flujo_Aire" 			var: airFlow_env ;
	parameter "Humedad" 			var: humidity_env ;
	parameter "Luminosidad" 		var: light_env 	;
	...
	output {
		...
		species arcilla aspect: default;			//LD
	 }
